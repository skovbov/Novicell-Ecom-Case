...
I en produktionsklar løsning ville der sidde en **load balancer** foran servicen der fordeler trafik på tværs af flere instanser under høj belastning som Black Friday. API'et er designet stateless, så horisontal skalering kan fungere uden ændringer i koden. Cachen ville i produktion være **Redis** frem for in-memory cache, så alle instanser deler samme cache-lag. Databasen ville hostes på en managed service som **Azure Database for PostgreSQL** der håndterer backup og failover automatisk.
